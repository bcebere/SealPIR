...
PIRServer::PIRServer(const EncryptionParameters &params,
                     const PirParams &pir_params)
    : params_(params), pir_params_(pir_params), is_db_preprocessed_(false) {
  ...
  for (int i = 0; i < encrypted_count; i++) {
    for (int j = 0; j < coeff_mod_count; j++) {
      negacyclic_shift_poly_coeffmod(
          ...
          params_.coeff_modulus()[j], destination.data(i) + (j * coeff_count));
    }
  }
}

inline void PIRServer::decompose_to_plaintexts_ptr(const Ciphertext &encrypted,
                                                   Plaintext *plain_ptr,
                                                   int logt) {
...
vector<Plaintext> PIRServer::decompose_to_plaintexts(
