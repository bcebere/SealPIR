...
vector<uint64_t> get_dimensions(uint64_t plaintext_num, uint32_t d) {
  ...
  for (uint32_t i = 0; i < d; i++) {
    dimensions[i] =
        std::max((uint32_t)2, (uint32_t)floor(pow(plaintext_num, 1.0 / d)));
  }
...
uint64_t plaintexts_per_db(uint32_t logtp, uint64_t N, uint64_t ele_num,
                           uint64_t ele_size) {
...
void coeffs_to_bytes(uint32_t limit, const Plaintext &coeffs, uint8_t *output,
                     uint32_t size_out) {
  uint32_t room = 8;
  uint32_t j = 0;
...
vector<Ciphertext> deserialize_ciphertexts(uint32_t count, string s,
                                           uint32_t len_ciphertext) {
  vector<Ciphertext> c;
  for (uint32_t i = 0; i < count; i++) {
    c.push_back(
        deserialize_ciphertext(s.substr(i * len_ciphertext, len_ciphertext)));
  }
  return c;
}
...
PirQuery deserialize_query(uint32_t d, uint32_t count, string s,
  ...
  for (uint32_t i = 0; i < d; i++) {
    c.push_back(deserialize_ciphertexts(
        count, s.substr(i * count * len_ciphertext, count * len_ciphertext),
        len_ciphertext));
  }
  return c;
}
